...
class SGSPLAG:
    ...
    def process(self):
        ...
        OUTPUT: type_plag <int> - Verbatim plagarism flag
                summary_flag <int> - summarization plagarism flag
        """
        self.preprocess()
    ...
    def compare(self):
        ...
        OUTPUT: detections <list> - Representation of plagairism cases before writing the xml file with require PAN format
                ...
                summary_flag <int> - summarization flag
        """
        detections = []
        ps = self.seeding()
        ...
        if type_plag == 0:
            ...
            if sum_src != 0 and sum_src >= 3 * sum_susp: #summarization heuristic
                summary_flag = 1
                for plag in plags2:
                    arg1 = (self.susp_offsets[plag[0][0]][0], self.susp_offsets[plag[0][1]][0] + self.susp_offsets[plag[0][1]][1])
                    arg2 = (self.src_offsets[plag[1][0]][0], self.src_offsets[plag[1][1]][0] + self.src_offsets[plag[1][1]][1])
...
def serialize_features(susp, src, features, outdir,outdir2,type_plag, summary_flag, source_text, susp_text):
    """
    ...
    if len(features):
        ...
        if len(dics):
            ...
            dic['para_pair'] = para_pair2
            dics.append(dic)
            with open(outdir2+str(src)+'-'+str(susp)+'.json', 'w') as file:
                json.dump(dics , file)
...
if __name__ == "__main__":
    ...
    args = parse_args()
    outdir_root="./"+args.type + '/'+args.dataset+'/'
    outdir="./"+args.type+'/'+args.dataset+'/all/'
    outdir_verbatim = "./"+args.type+'/'+args.dataset+'/verbatim/'
    outdir_summary ="./"+args.type+'/'+args.dataset+'/summary/'
    outdir_random_or_translate="./"+args.type+'/'+args.dataset+'/random_or_translate/'
    outdir_random_or_translate_low="./"+args.type+'/'+args.dataset+'/random_or_translate_low/'
    if not os.path.exists(outdir):
        os.makedirs(outdir)
    if not os.path.exists(outdir_verbatim):
        os.makedirs(outdir_verbatim)
    if not os.path.exists(outdir_summary):
        os.makedirs(outdir_summary)
    if not os.path.exists(outdir_random_or_translate):
        os.makedirs(outdir_random_or_translate)
    if not os.path.exists(outdir_random_or_translate_low):
        os.makedirs(outdir_random_or_translate_low)
    ...
    count_verbatim = 0
    count_summary = 0
    random_or_translate_confident = 0
    random_or_translate = 0


    # FILE="/Applications/LLM/code/LLM_test/GPT_generated_text/arxivGPT/arxivGPT_10000_temp.txt"
   # read the .txt file (consisting of machine-generated texts) of your interest and insert it into list
    dir= args.filedir
    # f2 = open(dir, 'r')
    # info_data = json.load(f2)
    # print(info_data)
    # lst_values = list(info_data.values())
    # lst_keys = list(info_data.keys())
    ...
    with open(dir, 'r') as file:
        for line in file:
            ...
            if bool(re.match(pattern, l)) == True and not l[0:2]=="0=":
                data.append(all_line)
                all_line = ""
            else:
                all_line = all_line + l
    data.append(all_line)
    ...
    USER="elastic"
    PASS="UONQRv12pOvEviXrspEk"
    CERTIFICATE="47bc3704bbaa8b7213842d872338eba22d4eb3d844c078554df1f926d9d200bc"
    ...
    prefix_past = []
    INDEX=args.index_xsuffix
    import ipdb
    # ipdb.set_trace()
    ii = 0
    accumulate_txt = ""

    df = pd.read_json("./gpt-neo-125m/" + args.dataset + "_prefix.json", orient='split')


    for i in range(len(data)):
        # ipdb.set_trace()
        # print(data[i], re.match(r'^\d+={23}$', data[i]))
        # if ii == 0 and re.match(r'^\d+={23}$', data[i]) is not None:
        #     ii = ii + 1
        # elif ii > 0 and re.match(r'^\d+={23}$', data[i]) is not None:
        #     print("next paragraph", ii, count_no, count_verbatim, count_summary, random_or_translate)
        #     ii = ii + 1
        # else:
        #     accumulate_txt = accumulate_txt + data[i]
        #     continue
        # preprocess search queries

        # gpt_txt = accumulate_txt
        # accumulate_txt = ""
        gpt_txt = data[i]
        gpt_prefix=df['prefix'][i]
        # print(gpt_prefix, gpt_txt)
        if(args.flag_decoder==1):
            gpt_txt=gpt_txt[len(gpt_prefix):]
            # print("gpt_txt",gpt_txt, gpt_prefix)

        times=0
        appear=0
        if gpt_prefix in prefix_past:
            times+=1
        prefix_past.append(gpt_prefix)
        ...
        # print("search end")
        # ipdb.set_trace()
        if len([hit["_source"] for hit in res['hits']['hits']])>0:
            elastic_results = [hit["_source"] for hit in res['hits']['hits']]
            # print(elastic_results)
        else: continue

        # Detect self-memorization
        ...
        for elastic_result in elastic_results:
            ...
            if len(sgsplag_obj.detections) == 0:
                ...
                # print(elastic_result['id'], i, sgsplag_obj.detections, outdir_verbatim, type_plag,summary_flag, gpt_txt, elastic_result['text'])
                appear+=1
                if appear==1 and times==0:
                    count_verbatim += 1
                serialize_features(elastic_result['id'], i, sgsplag_obj.detections, outdir_verbatim,outdir_random_or_translate_low,type_plag,
                                   summary_flag, gpt_txt, elastic_result['text'])
            elif len(sgsplag_obj.detections) and summary_flag:
                # print(elastic_result['id'], i, sgsplag_obj.detections, outdir_verbatim, type_plag, summary_flag,
                #       gpt_txt, elastic_result['text'])
                appear+=1
                if appear==1 and times==0:
                    count_summary += 1
                serialize_features(elastic_result['id'], i, sgsplag_obj.detections, outdir_summary,outdir_random_or_translate_low, type_plag,
                                   summary_flag, gpt_txt, elastic_result['text'])
            elif len(sgsplag_obj.detections) and not type_plag and not summary_flag:
                appear+=1
                if appear == 1 and times == 0:
                    random_or_translate+=1
                write=serialize_features(elastic_result['id'], i, sgsplag_obj.detections, outdir_random_or_translate,outdir_random_or_translate_low, type_plag,
                                   summary_flag, gpt_txt, elastic_result['text'])
                if appear == 1 and times == 0 and write>0:
                    random_or_translate_confident += 1
            # print(elastic_result['text_id'])
            serialize_features(elastic_result['id'], i, sgsplag_obj.detections, outdir, outdir_random_or_translate_low,type_plag, summary_flag, gpt_txt, elastic_result['text'])
        if i % 1000 == 0:
            print(args.dataset, i, count_no, count_verbatim, count_summary,random_or_translate_confident,random_or_translate)
    path=outdir_root+'result.txt'
    with open(path,'w') as f4:
        print(count_no,count_verbatim,count_summary,random_or_translate_confident,random_or_translate,file=f4)